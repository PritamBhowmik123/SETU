  ...
  return (
    <nav className="w-full bg-black/70 backdrop-blur-md shadow-md px-6 py-4 fixed top-0 left-0 z-[2000] flex items-center justify-between">
      ...
      {mobileMenuOpen && (
        <div
          ref={mobileMenuRef}
          className="absolute top-full left-0 w-full bg-black/95 backdrop-blur-xl border-t border-gray-800 shadow-2xl md:hidden flex flex-col items-center py-6 space-y-4 transition-all duration-300 z-[1900]"
        >
          {navLinks.map((link) => {
            if (!link || !link.name || !link.path) return null;
            return (
